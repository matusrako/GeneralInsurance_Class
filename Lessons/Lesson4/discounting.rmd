...
```{r}
Paid_HH_sml_triangle <- Paid_HH_sml %>% as.triangle(Paid_HH_sml, origin = "ay", dev = "dy", value = "SumOfamount")
```
...
Paid_HH_lrg_triangle <- Paid_HH_lrg %>% as.triangle(Paid_HH_lrg, origin = "ay", dev = "dy", value = "SumOfamount")
...
## 3rd Party & Large & PAID:
Paid_3rd_lrg <- dt_PaidCase %>% filter(Business == "3rd Party" & ClaimSize == "Large" & dataset_type == "PAID") 
Paid_3rd_lrg_triangle <- Paid_3rd_lrg %>% as.triangle(Paid_3rd_lrg, origin = "ay", dev = "dy", value = "SumOfamount")
...
# Veľmi výrazne vysoké straty sú v poslednom roku 2016, čo potrvdzuje aj predpoveď.

## 3rd Party & Small & PAID:
Paid_3rd_sml <- dt_PaidCase %>% filter(Business == "3rd Party" & ClaimSize == "Small")
Paid_3rd_sml_triangle <- Paid_3rd_sml %>% as.triangle(Paid_3rd_sml, origin = "ay", dev = "dy", value = "SumOfamount")
...
```
# V prípade House vidím, že po cca 4-5 rokov začínajú straty dramticky klesať, kým v prípade 3rd Party nie.
# Medzi veľkými a malými škodami som nezaznamenal dramatické rozdiely, ani v prípade House ani v prípade 3rd Party.
...
```{r}
## House & Small:
Incur_HH_sml <- dt_PaidCase %>% filter(Business == "House" & ClaimSize == "Small") 
Incur_HH_sml_triangle <- Incur_HH_sml %>% as.triangle(Incur_HH_sml, origin = "ay", dev = "dy", value = "SumOfamount")
Incur_HH_sml_triangle <- triangle(Incur_HH_sml_triangle[,10], Incur_HH_sml_triangle[,9], Incur_HH_sml_triangle[,1], Incur_HH_sml_triangle[,2], Incur_HH_sml_triangle[,3], Incur_HH_sml_triangle[,4], Incur_HH_sml_triangle[,5], Incur_HH_sml_triangle[,6], Incur_HH_sml_triangle[,7], Incur_HH_sml_triangle[,8])
plot(Incur_HH_sml_triangle)
plot(predict(chainladder(IncurHH_sml_triangle)))
ata(Incur_HH_sml_triangle)

## House & Large:
Incur_HH_lrg <- dt_PaidCase %>% filter(Business == "House" & ClaimSize == "Large") 
Incur_HH_lrg_triangle <- Incur_HH_lrg %>% as.triangle(Incur_HH_lrg, origin = "ay", dev = "dy", value = "SumOfamount")
Incur_HH_lrg_triangle <- triangle(Incur_HH_lrg_triangle[,10], Incur_HH_lrg_triangle[,9], Incur_HH_lrg_triangle[,1], Incur_HH_lrg_triangle[,2], Incur_HH_lrg_triangle[,3], Incur_HH_lrg_triangle[,4], Incur_HH_lrg_triangle[,5], Incur_HH_lrg_triangle[,6], Incur_HH_lrg_triangle[,7], Incur_HH_lrg_triangle[,8])
plot(Incur_HH_lrg_triangle)
plot(predict(chainladder(IncurHH_lrg_triangle)))
ata(Incur_HH_lrg_triangle)

## 3rd Party & Large:
Incur_3rd_lrg <- dt_PaidCase %>% filter(Business == "3rd Party" & ClaimSize == "Large") 
Incur_3rd_lrg_triangle <- Incur_3rd_lrg %>% as.triangle(Incur_3rd_lrg, origin = "ay", dev = "dy", value = "SumOfamount")
Incur_3rd_lrg_triangle <- triangle(Incur_3rd_lrg_triangle[,10], Incur_3rd_lrg_triangle[,9], Incur_3rd_lrg_triangle[,1], Incur_3rd_lrg_triangle[,2], Incur_3rd_lrg_triangle[,3], Incur_3rd_lrg_triangle[,4], Incur_3rd_lrg_triangle[,5], Incur_3rd_lrg_triangle[,6], Incur_3rd_lrg_triangle[,7], Incur_3rd_lrg_triangle[,8])
plot(Incur_3rd_lrg_triangle)

## 3rd Party & Small:
Incur_3rd_sml <- dt_PaidCase %>% filter(Business == "3rd Party" & ClaimSize == "Small") 
Incur_3rd_sml_triangle <- Incur_3rd_sml %>% as.triangle(Incur_3rd_sml, origin = "ay", dev = "dy", value = "SumOfamount")
Incur_3rd_sml_triangle <- triangle(Incur_3rd_sml_triangle[,10], Incur_3rd_sml_triangle[,9], Incur_3rd_sml_triangle[,1], Incur_3rd_sml_triangle[,2], Incur_3rd_sml_triangle[,3], Incur_3rd_sml_triangle[,4], Incur_3rd_sml_triangle[,5], Incur_3rd_sml_triangle[,6], Incur_3rd_sml_triangle[,7], Incur_3rd_sml_triangle[,8])
plot(Incur_3rd_sml_triangle)
plot(predict(chainladder(Incur_3rd_sml_triangle)))
ata(Incur_3rd_sml_triangle)
```
...
# Cca 4-5 rokov trvá strata Houses (dramaticky začne potom strata klesať), kým pre 3rd Party to trvá dlhšie. 
# Dôvod: Ak sa niečo stane s domom, je to väčšinou rýchlo zaplatené bez ďalších nákladov do budúcna - ide skôr o jednorázové plnenie, t. j. ľahšie chovsty.
# V prípade nehôd áut bývajú častejšie aj zdravotné škody (liečenie, práceneschopnosť), množstvo drobnejších vecí zasahujúcich do bežného chodu poškodeného, čo časovo výraznejšie predĺžuje vznik ďalších nákladov (ťažšie chvosty).
...
# Business = Travel:
dt_KPI_Tr <- dt_KPI %>% filter(Business == "Travel" & Segment == "Small")
dt_KPI_Tr_triangle <- dt_KPI_Tr %>% as.triangle(origin = "Year", dev = "sem netuším čo mám dať", value = "Losses")
plot(dt_KPI_Tr_triangle)
