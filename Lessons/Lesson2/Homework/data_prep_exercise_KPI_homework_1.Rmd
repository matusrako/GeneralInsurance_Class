...
# Najvyššiu Premium získala Unit7.
```
...
# Najvyššiu Premium získala Unit7. Najnižšie Expenses má Unit5, pričom z Unit5 má najvyššie Expenses business Travel.
...
dt_KPI_raw %>% 
  ...
  arrange(desc(Losses))
# Zle z hľadiska Losses vyzerá byť Unit2.
```
...
dt_KPI_raw %>% 
  ...
  summarize(Expenses = sum(Expenses, na.rm = TRUE)) %>% 
  arrange(desc(Expenses))
# Celkom dobre z hľadiska Expenses vyzerá byť Unit5.
```
...
dt_KPI_raw %>% 
  ...
  summarize(Expenses = sum(Expenses, na.rm = TRUE)) %>% 
  arrange(Expenses)
# Zlé Expenses z Unit5 vykazuje business Travel.
```
...
# Zle z hľadiska Losses vyzerá byť Unit2. Celkom dobre z hľadiska Expenses vyzerá byť Unit5, pričom z Unit5 zlé Expenses vykazuje business Travel.
...
dt_KPI_raw %>% 
  ...
  arrange(desc(UWR))
# Celkom dobre z hľadiska UWR vyzerá byť Unit7.
```
...
dt_KPI_raw %>% 
  ...
  arrange(UWR.Plan.)
# Zlý z hľadiska UWR z Unit7 je business Accidents (má zápornú hodnotu UWR).
```
...
# Celkom dobre z hľadiska UWR vyzerá byť Unit7, pričom zlá je business Accidents z dôvodu zápornej hodnoty UWR.
